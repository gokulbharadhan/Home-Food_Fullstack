...
const DeliveredOrder = () => {
 ...
 const filteredRecipes = dataList.filter((recipe) =>
        recipe.orderno.toLowerCase().includes(searchTerm.toLowerCase())
      );
    useEffect(() => {
        loadData();
    }, []);
    ...
    const loadData = async () => {
        try {
            ...
            console.log(response.data); // Check the structure and content
                    ...
                    <div className="row">
                        <div className="col-xl-12">
                            <div className="bradcam_text text-center">
                                <h3>Delivered and Canceled Orders</h3>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div className="customer_feedback_area pb-5" style={{marginTop:'-150px'}}>
                <div className="container">
                    <Grid container spacing={3}>
                        {filteredRecipes
                            .filter(order =>order.orderStatus !== "Pending" && order.orderStatus!=="Confirmed") // Filter out delivered orders
                            .map((order) => (
