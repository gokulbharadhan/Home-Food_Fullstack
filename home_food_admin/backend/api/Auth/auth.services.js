...
module.exports = {
    ...
    changepwd: (bodyData, callBack) => {
        if (bodyData.newPassword == bodyData.confirmPassword) {
            ...
            pool.query(
                `select password from user where email = ? and name = ?`,
                [email, userName],
                (err, results) => {
                    ...
                    if (err) {
                        ...
                        pool.query(
                            `update user set password = ? where email = ? and name=?`,//query
    ...
    updateUserById: (data, callBack) => {
        pool.query(
            ...
            [data.name,data.email,data.contact,data.id],//passing parameters
            (err, results) => {//sending response
                if (err) {
                    return callBack(err);
                } else if (results) {
